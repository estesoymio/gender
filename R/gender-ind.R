...
#
# This internal function implements the \code{method = "ind"} option of
...
gender_ind <- function(names) {
  ...
  apply_ind <- function(n) {
    ...
    results <- genderdata::genderind %>% filter(name == tolower(n))
  ...
  if (length(names) == 1) {
    return(apply_ind(names))
  } else {
    return(bind_rows(lapply(names, apply_ind)))
  }
